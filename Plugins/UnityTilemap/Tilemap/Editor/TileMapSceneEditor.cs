...
namespace toinfiniityandbeyond.Tilemapping
{
	partial class TileMapEditor : Editor
	{
		...
		EditorType display = EditorType.All;
		...
		private void OnSceneGUI ()
		{
			if (tileMap.isInEditMode)
			{
				...
				if (tileMap.primaryTilePickerToggle)
				{
					...
					tileMap.tilePickerWindowPosition = ClampToScreen (GUILayout.Window (GUIUtility.GetControlID (FocusType.Passive), tileMap.tilePickerWindowPosition, (int id) => { TilepickerWindow (id, ref tileMap.primaryTile); }, new GUIContent ("Pick primary tile..."), GUILayout.Width (200)));
				}
				else if (tileMap.secondaryTilePickerToggle)
				{
					...
					tileMap.tilePickerWindowPosition = ClampToScreen (GUILayout.Window (GUIUtility.GetControlID (FocusType.Passive), tileMap.tilePickerWindowPosition, (int id) => { TilepickerWindow (id, ref tileMap.secondaryTile); }, new GUIContent ("Pick secondary tile..."), GUILayout.Width (200)));
				}
		...
		private void TilepickerWindow (int id, ref ScriptableTile tileToChange)
		{
			...
			{
				RefreshScriptableTileCache (display);
			}
			...
			display = (EditorType)EditorGUILayout.EnumPopup(
                "Filter:",
                display);
			...
			if (tileMap.selectedScriptableTool >= 0 && tileMap.selectedScriptableTool < tileMap.scriptableToolCache.Count)
			{
				if (e.button == 0)
				{
					...
					for(int i = 0; i < region.Count; i++)
					{
						...
						Handles.CubeHandleCap (0, (Vector2)cursorPoint + Vector2.one * 0.5f, Quaternion.identity, 1f, EventType.Layout);
